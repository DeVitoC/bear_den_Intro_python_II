...
"inspect [item] = view description of item"
...
while True: 
    ...
    if len(commands) == 1:
        ...
        if commands[1].lower() == "north":
            ...
            if commands[1].lower() == "room":
                if player.current_room.stuff == []:
                    print("There is nothing in this room.")
                else:
                    print(player.current_room.stuff[commands[1]])
                    # for item in player.current_room.stuff:
                    #     print(item)
            else:
            ...
            for item in player.current_room.stuff.values():
                if item.name == commands[1]:
                    player.inventory.append(item)
                    player.current_room.stuff[item.name] = None
                    item.on_take()
        elif commands[0].lower() == "drop":
            for item in player.inventory:
                if item.name == commands[1]:
                    player.inventory.remove(item)
                    player.current_room.stuff[item.name] = item
                    item.on_drop()
